...
\begin{definition}
Задано $C$ -- категорія.\\
Морфізм $\alpha \colon x \to y$ називається \textbf{розщепленим мономорфізмом} (\text{split monic}), якщо
\begin{align*}
...
\begin{proof}
Нехай $C$ -- конкретна категорія та $\alpha \colon X \to Y$ -- розщеплений мономорфізм, тобто існує морфізм $\beta \colon Y \to X$, для якого $\beta \alpha = 1_X$. Тоді\\
...
\begin{theorem}
У категорія $\Set, \Top, \Grp$ морфізм сюр'єктивний $\iff$ морфізм -- епіморфізм.
\end{theorem}
